...
export function isDmOpenFor(agent) {
    if (!window.__currentView) return false;
    return window.__currentView === 'dm' && window.__currentDM === agent;
}
    ...
    chatChannel = supabase.channel('chat-room')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, payload => {
            ...
            if (chatTabActive === 'general' && window.__currentView === 'chat') {
                ...
                glowIcon('icon-chat');
                if (msg.author !== CA?.name) showChannelDot('chat');
            }
        })
...
// ============================================================
// ТОЧКИ НЕПРОЧИТАННОГО НА КАНАЛАХ
// ============================================================
let channelDots = JSON.parse(localStorage.getItem('syndicate_channel_dots') || '{}');

export function showChannelDot(channel) {
    if (!channel) return;
    channelDots[channel] = true;
    localStorage.setItem('syndicate_channel_dots', JSON.stringify(channelDots));
    renderChannelDots();
}

export function hideChannelDot(channel) {
    if (!channel) return;
    if (channelDots[channel]) {
        delete channelDots[channel];
        localStorage.setItem('syndicate_channel_dots', JSON.stringify(channelDots));
        renderChannelDots();
    }
}

function renderChannelDots() {
    let cv = window.__currentView || '';

    let generalTab = document.querySelector('.chat-tab[data-tab="general"]');
    if (generalTab) {
        let existing = generalTab.querySelector('.tab-dot');
        if (channelDots.chat && chatTabActive !== 'general') {
            if (!existing) generalTab.insertAdjacentHTML('beforeend', '<span class="tab-dot"></span>');
        } else if (existing) existing.remove();
    }

    let clanTab = document.querySelector('.chat-tab[data-tab="clan"]');
    if (clanTab) {
        let existing = clanTab.querySelector('.tab-dot');
        if (channelDots.clan && chatTabActive !== 'clan') {
            if (!existing) clanTab.insertAdjacentHTML('beforeend', '<span class="tab-dot"></span>');
        } else if (existing) existing.remove();
    }

    let dmIcon = document.querySelector('.sidebar-item[data-app="dm"] .sidebar-item-icon');
    if (dmIcon) {
        let existing = dmIcon.querySelector('.channel-dot');
        if (channelDots.dm && cv !== 'dm') {
            if (!existing) dmIcon.insertAdjacentHTML('beforeend', '<span class="channel-dot"></span>');
        } else if (existing) existing.remove();
    }

    let rpIcon = document.querySelector('.sidebar-item[data-app="rp"] .sidebar-item-icon');
    if (rpIcon) {
        let existing = rpIcon.querySelector('.channel-dot');
        if (channelDots.rp && cv !== 'rp') {
            if (!existing) rpIcon.insertAdjacentHTML('beforeend', '<span class="channel-dot"></span>');
        } else if (existing) existing.remove();
    }

    let announceIcon = document.querySelector('.sidebar-item[data-app="announce"] .sidebar-item-icon');
    if (announceIcon) {
        let existing = announceIcon.querySelector('.channel-dot');
        if (channelDots.announce && cv !== 'announce') {
            if (!existing) announceIcon.insertAdjacentHTML('beforeend', '<span class="channel-dot"></span>');
        } else if (existing) existing.remove();
    }
}

export function renderChannelDotsPublic() { renderChannelDots(); }

// ============================================================
// УВЕЛИЧЕНИЕ СЧЁТЧИКА НЕПРОЧИТАННОГО
// ============================================================
export function bumpUnread(channel) {
    if (channel === 'dm') unreadMentions.dm++;
    else if (channel === 'chat') unreadMentions.chat++;
    else if (channel === 'clan') unreadMentions.clan++;
    updateBadgeIcons();
}
...
export function renderChat(keepScroll = false) {
    ...
    c.innerHTML = msgs.map((m, i) => {
        ...
        let cs = m.author_color
            ? (getActiveColorClassForId(m.author_color) === 'rainbow-text' ? 'rainbow-text' : getActiveColorClassForId(m.author_color))
            : '';
        ...
        Object.keys(react).forEach(k => {
            ...
            rh += '<span class="chat-reaction ' + (ia ? 'active' : '') +
                '" data-reaction="chat" data-msgid="' + m.id + '" data-emoji="' + k + '">' +
                k + ' ' + react[k] +
...
export async function loadAdminMessages() {
    try {
        let { data } = await supabase.from('admin_messages').select('*').order('id', { ascending: false }).limit(50);
...
export function renderAdminChat() {
    ...
    c.innerHTML = msgs.map(m => {
        ...
        Object.keys(react).forEach(k => {
            ...
            let ia = Array.isArray(ua) && CA && ua.includes(CA.name);
            rh += '<span class="chat-reaction ' + (ia ? 'active' : '') +
                '" data-reaction="admin" data-msgid="' + m.id + '" data-emoji="' + k + '">' +
                k + ' ' + react[k] +
                '<span class="chat-reaction-tooltip">' + (us || '...') + '</span></span>';
        });
        ...
        return '<div class="chat-msg"><div class="chat-msg-left"><span class="chat-avatar-frame f-default"><span class="inner">' + avatarHtml + '</span></span></div>' +
            '<div class="chat-msg-right"><div class="chat-header-row">' +
            '<span class="chat-author">' + m.author + '</span>' +
            '<span class="chat-time">' + m.time + '</span>' + menu + '</div>' +
            '<div class="chat-text">' + txt + '</div>' +
            '<div class="chat-reactions">' + rh + '<span class="chat-reaction" data-reaction-picker="admin" data-msgid="' + m.id + '">+</span></div>' +
            '</div></div>';
    }).join('');
...
export async function loadClanMessages(clanId) {
    try {
        let { data } = await supabase.from('clan_messages').select('*').eq('clan_id', clanId).order('id', { ascending: true }).limit(50);
    ...
    clanChannel = supabase.channel('clan-' + clanId)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'clan_messages', filter: 'clan_id=eq.' + clanId }, payload => {
            ...
            if (payload.new.text && payload.new.text.includes('@' + CA?.name) && payload.new.author !== CA?.name) {
                unreadMentions.clan++;
                updateBadgeIcons();
                notif('📢 УПОМЯНУЛИ В ОТРЯДЕ');
            }
            if (currentClanId == clanId && chatTabActive === 'clan') renderClanMessages();
            else if (payload.new.author !== CA?.name) showChannelDot('clan');
        })
        .subscribe();
}
...
export function renderClanMessages() {
    ...
    if (msgs.length === 0) {
        c.innerHTML = '<div style="text-align:center;color:#880000;padding:20px;">НЕТ СООБЩЕНИЙ</div>';
        return;
    }
    ...
    c.innerHTML = msgs.map(m => {
        ...
        Object.keys(react).forEach(k => {
            ...
            let ia = Array.isArray(ua) && CA && ua.includes(CA.name);
            rh += '<span class="chat-reaction ' + (ia ? 'active' : '') +
                '" data-reaction="clan" data-msgid="' + m.id + '" data-emoji="' + k + '">' +
                k + ' ' + react[k] +
                '<span class="chat-reaction-tooltip">' + (us || '...') + '</span></span>';
        });
        ...
        return '<div class="chat-msg' + (m.pinned ? ' pinned' : '') + '" data-msg-id="' + m.id + '">' +
            '<div class="chat-msg-left"><span class="chat-avatar-frame ' + frc + '"><span class="inner">' + avatarHtml + '</span></span></div>' +
            '<div class="chat-msg-right"><div class="chat-header-row">' +
            '<span class="chat-author ' + cs + '" onclick="window.showAgentInfo(\'' + m.author + '\')" style="cursor:pointer;">' + (m.author || '???') + '</span>' +
            '<span class="chat-time">' + (m.time || '') + '</span>' + menu + '</div>' +
            (m.pinned ? '<div style="color:var(--warning);font-size:0.75rem;">📌 Закреплено</div>' : '') +
            '<div class="chat-text ' + fc + '">' + txt + '</div>' +
...
function renderClanList() {
    ...
    setTimeout(() => {
        document.querySelectorAll('[data-open-clan]').forEach(b =>
            b.addEventListener('click', function() { openClanChat(parseInt(this.dataset.openClan)); })
        );
    }, 10);
}
