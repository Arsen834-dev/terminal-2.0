...
export function isAchievementUnlocked(id) {
    return CA?.achievements?.includes(id) || false;
}

export function getAchievementProgress() {
    return {
        unlocked: CA?.achievements?.length || 0,
...
export function checkAchievements() {
    ...
    if (!CA.achievements.includes('first_login')) {
        unlockAchievement('first_login');
    }
    if ((CA.guidesCreated || 0) >= 1 && !CA.achievements.includes('guide_master')) {
        unlockAchievement('guide_master');
    }
    if ((CA.chatCount || 0) >= 10 && !CA.achievements.includes('socializer')) {
        unlockAchievement('socializer');
    }
    if ((CA.crystals || 0) >= 1000 && !CA.achievements.includes('rich')) {
        unlockAchievement('rich');
    }
    if ((CA.rep || 0) >= 100 && !CA.achievements.includes('reputation')) {
        unlockAchievement('reputation');
    }
    if ((CA.clansCreated || 0) >= 1 && !CA.achievements.includes('clan_creator')) {
        unlockAchievement('clan_creator');
    }
}
