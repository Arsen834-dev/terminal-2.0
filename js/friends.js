...
export function renderFriends() {
    loadFriends().then(() => {
        ...
        if (accepted.length === 0) c.innerHTML += '<div style="color:#cc0000;">НЕТ ДРУЗЕЙ</div>';
            ...
            c.innerHTML += '<div class="admin-row"><span>🤝 ' + n + '</span><div><button class="modal-btn" style="font-size:0.8rem;" data-dm-friend="' + n + '">📁</button><button class="modal-btn" style="font-size:0.8rem;" data-remove-friend="' + f.id + '">🗑</button></div></div>';
        });

        if (myReq.length > 0) {
            c.innerHTML += '<div style="margin-top:15px;color:#ffd700;">📤 ОТПРАВЛЕННЫЕ:</div>';
            myReq.forEach(f => {
                c.innerHTML += '<div class="admin-row"><span>📤 ' + f.friend + '</span><button class="modal-btn" style="font-size:0.8rem;" data-remove-friend="' + f.id + '">🗑</button></div>';
            });
        }

        if (theirReq.length > 0) {
            c.innerHTML += '<div style="margin-top:15px;color:#00ff41;">📥 ВХОДЯЩИЕ:</div>';
            theirReq.forEach(f => {
                c.innerHTML += '<div class="admin-row"><span>📩 ' + f.agent + '</span><div><button class="modal-btn" style="font-size:0.8rem;" data-accept-friend="' + f.id + '">✅</button><button class="modal-btn" style="font-size:0.8rem;color:#ff0000;border-color:#ff0000;" data-remove-friend="' + f.id + '">✕</button></div></div>';
            });
        }
        ...
        setTimeout(() => {
            document.getElementById('add-friend-btn')?.addEventListener('click', async function() {
                ...
                if (r.success) { notif(r.message || '🤝 ЗАПРОС ОТПРАВЛЕН'); renderFriends(); }
                else notif(r.error);
            });
